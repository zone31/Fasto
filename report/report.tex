...
| `/`   { Parser.DIVIDE   (getPos lexbuf) }
    ...
    defined, as integer multiplication and division has a lot in common with
...
| evalExp ( Divide(e1, e2, pos), vtab, ftab ) =
    ...
    expected type.  If the types match, the types are returned with
...
| Divide (e1, e2, pos) =>
    ...
    we made sure that the boolean operators where defined as having a lower
...
End:
    ...
    right hand side and jump straight to False.  We did something similar for
...
fun keyword (s, pos) =
    ...
    constants which are negated, the negated value is simply created on
...
| MAP LPAR FunArg COMMA Exp RPAR { Map ($3, $5, (), (), $1) }
    ...
    meaning that the lambda is supposed to be passed as a \texttt{FunArg}.  We then
...
the table is declared twice, and at last we check if the return type of the function actually is correctly defined.
...
\begin{enumerate}
 ...
 This makes the expression able to see variables declared outside the scope  
  ...
  local vTable, so the function can reassign variables such as x, and a global, so as to call
...
\subsection{Tests}
    ...
    \begin{tabular}{|l|l|}
        ...
        \hline
        lambda\_nested\_sameName.fo        & No variable conflicts, no double decelerations       \\
        \hline
...
fun bindExpPropagatee name e vtable =
	...
	\subsection{Tests}
	We have tested our optimization implementations by using the commands: \\
